...
eval(char *cmdline) 
{
    //char *argv[MAXARGS];//argument to execve()
    ...
    int fin=-1,fout=-1;//储存标准输入输出
    int nohup=0;//nohup flag
    ...
    if(tok.outfile)
    {
        int fd = open(tok.outfile,O_WRONLY|O_CREAT|O_TRUNC);
        fout=dup(1);
        dup2(fd,1);
        close(fd);
    }
    if(tok.builtins==BUILTIN_NOHUP)//nohup process
    {
        nohup=1;
    }
    if(!builtin_cmd(&tok))
    {
        ...
        {
            Setpgid(0, 0);
            if(nohup)//ignore SIGHUP, change argv
            {
        ...
        sigprocmask(SIG_SETMASK, &mask_prev, NULL); //addjob执行完解除阻塞
        //fg则等待前台运行完成
        if(!bg)
        {
 ...
 */
void 
sigchld_handler(int sig) //reap zombies process
{
    ...
    while((pid=waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0)
    {
        ...
        else if(WIFSIGNALED(status)) //因信号而终止
        {
            sio_put("Job [%d] (%d) terminated by signal %d\n",pid2jid(pid), pid, WTERMSIG(status));
            deletejob(job_list, pid);
        }
        else if(WIFSTOPPED(status)) //因信号而停止
        {
